...
if [[ "$1" == "debug" ]]; then
  # debug build
  $CPP $SRC $INCLUDE_DIRS $DEFINES $LIBS -o "$BUILD_DIR/$NAME.exe" -g -fexceptions || exit 1
else
  # release build
  $CPP $SRC $INCLUDE_DIRS $DEFINES $LIBS -o "$BUILD_DIR/$NAME.exe" -fexceptions -Os -s || exit 1  # -flto
